...
async function main() {
  ...
  const product = await productsRepository.create({
    name: `Validate BXGY generic product ${suffix}`,
    slug: `validate-bxgy-generic-${suffix}`,
    sku: `VAL-BXGY-GEN-${suffix}`,
    price: "10.00",
  ...
  await discountsRepository.create({
    name: `Validate BXGY generic ${suffix}`,
    code: null,
  ...
  const [cart] = await db
    .insert(schema.carts)
    .values({
      sessionId: `validate-bxgy-generic-${suffix}`,
      status: "active" as any,
      currency: "CAD",
    } as any)
    .returning();
  ...
  assertApprox("subtotal", subtotal, 30);
  assertApprox("discountAmount", discountAmount, 10);
  assertApprox("totalAmount", totalAmount, 20);

  const [created] = await db
    .select({ id: schema.discounts.id })
    .from(schema.discounts)
    .where(sql`${schema.discounts.name} = ${`Validate BXGY generic ${suffix}`}`)
    .limit(1);
  if (created?.id) {
    await db
  ...
  process.stdout.write("OK: BXGY generic validated.\n");
}
