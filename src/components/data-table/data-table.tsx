  ...
  return table.getRowModel().rows.map((row) => (
    <TableRow
      ...
      className={cn(
        ...
        getRowClassName ? getRowClassName(row.original as TData) : undefined,
      )}
    >
...
export function DataTable<TData, TValue>({
  ...
  const dataIds: UniqueIdentifier[] = table.getRowModel().rows.map((row) => row.id as unknown as UniqueIdentifier);
  ...
  const tableContent = (
    <Table>
      ...
      <TableBody className="**:data-[slot=table-cell]:first:w-8">
        {renderTableBody({ table, columns, dndEnabled, dataIds, onRowClick, getRowClassName })}
      </TableBody>
    </Table>
  );
