...
export function ChartAreaInteractive() {
  ...
  const [series, setSeries] = React.useState<Array<{ date: string; revenue: number; profit: number; orders: number }>>([]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/v1/dashboard/timeseries?range=${timeRange}`, { cache: "no-store" });
        ...
        <ChartContainer config={chartConfig} className="aspect-auto h-[340px] w-full">
          <AreaChart data={series}>
            ...
            <XAxis
              ...
              tickFormatter={(value) => {
                try {
                  return new Intl.DateTimeFormat(locale || undefined, { month: "short", day: "numeric" }).format(new Date(value));
                } catch {
                  return String(value);
                }
              }}
